...
│   ├── internal/
│   │   ├── config/          # 配置加载
│   │   ├── handler/         # HTTP 路由处理器（5 个 handler）
...
│   ├── migrations/          # SQL 迁移脚本（4 个 up/down）
│   ├── config.yaml          # 默认配置
│   ├── Dockerfile
│   └── docker-compose.yml
├── frontend/
...
服务启动后，API 监听在 `http://www.olraingin.com:8080`。MySQL 数据库会自动初始化并运行迁移脚本。
...
flutter pub get
# 连接服务器（替换为你的实际地址）
flutter run --dart-define=BASE_URL=http://www.olraingin.com:8080
```
...
| GET | `/api/accounts` | 账户列表 | 是 |
| PUT | `/api/accounts/:id` | 修改账户 | 是 |
| POST | `/api/categories` | 创建分类 | 是 |
| GET | `/api/categories` | 分类列表 | 是 |
| PUT | `/api/categories/:id` | 修改分类 | 是 |
| POST | `/api/transactions` | 创建流水 | 是 |
| GET | `/api/transactions` | 流水列表（分页） | 是 |
| GET | `/api/transactions/:id` | 流水详情 | 是 |
| PUT | `/api/transactions/:id` | 修改流水 | 是 |
| DELETE | `/api/transactions/:id` | 删除流水（软删除） | 是 |
| GET | `/api/stats/monthly` | 月度统计 | 是 |
